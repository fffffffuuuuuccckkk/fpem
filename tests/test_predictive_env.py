...
def test_gradient_relative_maturity_uses_rms_not_parameter_count():
    ...
    assert torch.allclose(g_inv, torch.tensor(1.0))
    assert torch.allclose(g_var, torch.tensor(1.0))
    assert torch.allclose(maturity, torch.tensor(0.5))
