...
class Dataset_SplitNpy(Dataset):
    """Adapter for /data/OuXiaoyu/mystg/datasets style split .npy datasets."""

    def __init__(self, args, root_path, flag='train', size=None,
                 features='M', data_path='', target='OT', scale=True, timeenc=0,
        ...
        self.root_path = root_path
        self.scaler = StandardScaler()
        train = np.load(os.path.join(root_path, 'train_data.npy')).astype('float32')
        cur = np.load(os.path.join(root_path, f'{flag}_data.npy')).astype('float32')
        ts_path = os.path.join(root_path, f'{flag}_timestamps.npy')
        if os.path.exists(ts_path):
            stamp = np.load(ts_path).astype('float32')
        else:
            stamp = np.zeros((cur.shape[0], 4), dtype='float32')
        if scale:
