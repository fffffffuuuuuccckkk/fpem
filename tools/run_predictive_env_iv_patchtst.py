...
def loss_gradient_norm(loss, parameters):
    """Detached L2 gradient norm for the established parameter subset."""
...
def gradient_relative_maturity(
    ...
    invariant_norm = loss_gradient_norm(invariant_loss, invariant_parameters)
    variant_norm = loss_gradient_norm(full_loss, variant_parameters)
    maturity = (
...
def build_training_optimizer(model, args):
    ...
    learning_rates = {
        ...
        "gamma_beta": (
            args.lr_gamma_beta_base
            if args.fixed_gamma_beta_lr
            else 0.2 * args.lr_gamma_beta_base
        ),
...
def update_gamma_beta_learning_rate(optimizer, maturity, args):
    ...
    for group in optimizer.param_groups:
        if group.get("name") == "gamma_beta":
            group["lr"] = target
        elif group.get("name") == "variant" and args.adaptive_variant_lr:
            group["lr"] = args.lr_variant * (
                0.2 + 0.8 * float(torch.as_tensor(maturity).detach().cpu())
            )
    return target
...
def invariant_anchored_variant_loss(
    ...
    variant_total = full_loss + float(lambda_anchor) * anchor_loss
...
def patch_correction_usefulness_target(
    target, invariant_prediction, raw_prediction, future_patch_len
):
    """Binary target: whether the raw correction improves each future patch."""
    residual = target - invariant_prediction.detach()
    correction = raw_prediction - invariant_prediction.detach()
    targets = []
    for start in range(0, target.shape[1], int(future_patch_len)):
        ...
        residual_patch = residual[:, start:end]
        correction_patch = correction[:, start:end]
        invariant_error = residual_patch.square().mean((1, 2))
        corrected_error = (residual_patch - correction_patch).square().mean((1, 2))
        targets.append((corrected_error < invariant_error).float())
    return torch.stack(targets, dim=1).detach()


def reliability_pairwise_ranking_loss(prediction, target):
    """Deterministic pairwise ranking loss that ignores tied analytic targets."""
    prediction = prediction.flatten()
    target = target.flatten()
    paired_prediction = prediction.roll(1)
    paired_target = target.roll(1)
    direction = torch.sign(target - paired_target)
    valid = direction != 0
    if not bool(valid.any()):
        return prediction.sum() * 0.0
    return F.softplus(
        -direction[valid] * (prediction[valid] - paired_prediction[valid])
    ).mean()
...
def controlled_environment_assignment(assignment, mode, seed, stage):
    """Return a deterministic environment-supervision control.

    Predictive EIIL discovery is always run first.  Controls alter only the
    detached supervision consumed during the following representation stage.
    They therefore keep model size, training data and update cadence fixed.
    """
    assignment = np.asarray(assignment, dtype=np.float32)
    if mode == "predictive":
        return assignment
    if assignment.ndim != 2 or assignment.shape[1] < 2:
        raise ValueError("environment assignment must have shape [N,K], K>=2")
    rng = np.random.RandomState(int(seed) + 104729 * (int(stage) + 1))
    if mode == "shuffled":
        return assignment[rng.permutation(len(assignment))].copy()
    if mode == "uniform":
        return np.full_like(assignment, 1.0 / assignment.shape[1])
    if mode == "random":
        labels = np.arange(len(assignment), dtype=np.int64) % assignment.shape[1]
        rng.shuffle(labels)
        result = np.zeros_like(assignment)
        result[np.arange(len(result)), labels] = 1.0
        return result
    raise ValueError(f"unknown environment supervision: {mode}")
...
def model_config(args, experiment):
    return SimpleNamespace(
        ...
        horizon_reliability_gate=args.horizon_reliability_gate,
        reliability_environment_disagreement=(
            args.reliability_environment_disagreement
        ),
...
def collect(
    ...
    result = [prediction_tensor, torch.cat(targets), torch.cat(ids)]
    if components:
        result.extend(
            [
                torch.cat(z_inv),
    ...
    if h_diagnostics:
        # When H itself is the requested prediction, reuse the concatenated
        # tensor instead of materializing a second multi-GB copy on datasets
        # such as Traffic.
        h_prediction_tensor = (
            prediction_tensor
            if prediction_source == "h_reference"
            else torch.cat(h_predictions)
        )
        result.extend([h_prediction_tensor, torch.cat(h_norms)])
    return result
...
def train_epoch(
    ...
    reliability_fraction_mse_values = []
    reliability_ranking_loss_values = []
    reliability_usefulness_bce_values = []
    reliability_usefulness_accuracy_values = []
    ...
    fixed_reliability_sample_ids = []
    fixed_reliability_targets = []
    raw_variant_gain_values = []
    gated_variant_gain_values = []
    gated_full_prediction_values = []
    invariant_environment_correct = 0
    variant_environment_correct = 0
    environment_label_count = 0
    raw_variant_effect_bin_values = []
    gated_variant_effect_bin_values = []
    correction_cosine_values = []
    correction_beneficial_values = []
    correction_magnitude_values = []
    environment_correction_loss_values = []
    environment_disagreement_values = []
    ...
    for step, batch in enumerate(loader):
        ...
        if output["horizon_environment_disagreement"] is not None:
            environment_disagreement_values.append(
                output["horizon_environment_disagreement"].detach().cpu()
            )
        isolate_z_gradients = (
        ...
        if model.variant_fusion_mode == "horizon_future_var":
            ...
            if args.differential_lr and not args.fixed_gamma_beta_lr:
                update_gamma_beta_learning_rate(
                    optimizer, maturity, args
                )
            model._last_gradient_maturity = {
                "maturity": lambda_maturity,
                "G_inv_L2": float(invariant_gradient.cpu()),
                "G_var_L2": float(variant_gradient.cpu()),
            }
            ...
            reliability_fraction_mse = F.mse_loss(
                reliability_prediction, reliability_target
            )
            usefulness_target = patch_correction_usefulness_target(
                y,
                output["invariant_prediction"],
                output["prediction"],
                args.future_patch_len,
            )
            reliability_ranking = reliability_pairwise_ranking_loss(
                reliability_prediction, reliability_target
            )
            reliability_usefulness_bce = F.binary_cross_entropy(
                reliability_prediction.clamp(1e-6, 1.0 - 1e-6),
                usefulness_target,
            )
            if args.reliability_objective == "mse":
                reliability_loss = reliability_fraction_mse
            elif args.reliability_objective == "mse_ranking":
                reliability_loss = (
                    reliability_fraction_mse + reliability_ranking
                )
            else:
                reliability_loss = (
                    reliability_fraction_mse + reliability_usefulness_bce
                )
            invariant_anchor = output["invariant_prediction"].detach()
            raw_correction = output["prediction"] - invariant_anchor
            residual_for_correction = y - invariant_anchor
            for patch_start in range(
                0, args.pred_len, int(args.future_patch_len)
            ):
                patch_end = min(
                    patch_start + int(args.future_patch_len), args.pred_len
                )
                residual_patch = residual_for_correction[
                    :, patch_start:patch_end
                ].flatten(1)
                correction_patch = raw_correction[
                    :, patch_start:patch_end
                ].flatten(1)
                dot = (residual_patch * correction_patch).sum(1)
                residual_norm = residual_patch.square().sum(1).sqrt()
                correction_square = correction_patch.square().sum(1)
                correction_norm = correction_square.sqrt()
                correction_cosine_values.append(
                    (
                        dot
                        / (residual_norm * correction_norm).clamp_min(1e-8)
                    ).detach().cpu()
                )
                correction_beneficial_values.append(
                    (2.0 * dot > correction_square).float().detach().cpu()
                )
                correction_magnitude_values.append(
                    (
                        correction_norm / residual_norm.clamp_min(1e-8)
                    ).detach().cpu()
                )
            ...
            gated_full_prediction_values.extend(
                gated_sample_loss.detach().cpu().tolist()
            )
            ...
            reliability_fraction_mse_values.append(
                float(reliability_fraction_mse.detach())
            )
            reliability_ranking_loss_values.append(
                float(reliability_ranking.detach())
            )
            reliability_usefulness_bce_values.append(
                float(reliability_usefulness_bce.detach())
            )
            reliability_usefulness_accuracy_values.append(float(
                (
                    (reliability_prediction.detach() >= 0.5)
                    == (usefulness_target >= 0.5)
                ).float().mean()
            ))
            ...
            fixed_mask = sample_id < min(int(args.diagnostic_samples), 256)
            if bool(fixed_mask.any()):
                fixed_reliability_sample_ids.append(sample_id[fixed_mask].cpu())
                fixed_reliability_targets.append(
                    reliability_target.detach()[fixed_mask.to(device)].cpu()
                )
        ...
        if model.level != "baseline":
            ...
            if model.level == "representation":
                if output["horizon_environment_corrections"] is not None:
                    environment_corrections = output[
                        "horizon_environment_corrections"
                    ]
                    correction_horizon = environment_corrections.flatten(
                        2, 3
                    )[:, :, : args.pred_len]
                    environment_predictions = (
                        output["invariant_prediction"].detach().unsqueeze(1)
                        + correction_horizon
                    )
                    environment_correction_error = (
                        environment_predictions - y.unsqueeze(1)
                    ).square().mean((2, 3))
                    environment_correction_loss = (
                        q * environment_correction_error
                    ).sum(dim=1).mean()
                    loss = loss + environment_correction_loss
                    environment_correction_loss_values.append(
                        float(environment_correction_loss.detach())
                    )
                ...
                if args.representation_constraint == "contrastive":
                    ...
                    domain_loss, domain_details = model.environment_classification_loss(
                        environment_output["z_inv"],
                        environment_output["z_var"],
                        q,
                    )
                    target_label = q.argmax(dim=-1)
                    invariant_environment_correct += int(
                        (domain_details["inv_env_logits"].detach().argmax(-1)
                         == target_label).sum()
                    )
                    variant_environment_correct += int(
                        (domain_details["var_env_logits"].detach().argmax(-1)
                         == target_label).sum()
                    )
                    environment_label_count += target_label.numel()
        ...
        result.update(
            {
                ...
                "raw_full_MSE": float(epoch_full.mean()),
                "gated_full_MSE": (
                    float(np.mean(gated_full_prediction_values))
                    if gated_full_prediction_values else float(epoch_full.mean())
                ),
    ...
    result["inv_acc"] = (
        invariant_environment_correct / environment_label_count
        if environment_label_count else float("nan")
    )
    result["var_acc"] = (
        variant_environment_correct / environment_label_count
        if environment_label_count else float("nan")
    )
    result["loss/inv"] = (
    ...
    result["G_inv"] = result["G_inv_L2"]
    result["G_var"] = result["G_var_L2"]
    for group_name in (
    ...
    variant_lrs = learning_rate_values["variant"]
    result["lr/variant_min"] = (
        float(np.min(variant_lrs)) if variant_lrs else float("nan")
    )
    result["lr/variant_max"] = (
        float(np.max(variant_lrs)) if variant_lrs else float("nan")
    )
    ...
    if environment_disagreement_values:
        disagreement = torch.cat(environment_disagreement_values).flatten()
        result["reliability_environment_disagreement_mean"] = float(
            disagreement.mean()
        )
        result["reliability_environment_disagreement_p90"] = float(
            torch.quantile(disagreement, 0.9)
        )
    else:
        result["reliability_environment_disagreement_mean"] = float("nan")
        result["reliability_environment_disagreement_p90"] = float("nan")
    ...
    result["reliability_objective"] = args.reliability_objective
    result["reliability_fraction_mse"] = (
        float(np.mean(reliability_fraction_mse_values))
        if reliability_fraction_mse_values else float("nan")
    )
    result["reliability_ranking_loss"] = (
        float(np.mean(reliability_ranking_loss_values))
        if reliability_ranking_loss_values else float("nan")
    )
    result["reliability_usefulness_bce"] = (
        float(np.mean(reliability_usefulness_bce_values))
        if reliability_usefulness_bce_values else float("nan")
    )
    result["reliability_usefulness_accuracy"] = (
        float(np.mean(reliability_usefulness_accuracy_values))
        if reliability_usefulness_accuracy_values else float("nan")
    )
    result["reliability_environment_correction_loss"] = (
        float(np.mean(environment_correction_loss_values))
        if environment_correction_loss_values else float("nan")
    )
    if environment_disagreement_values:
        disagreement = torch.cat(environment_disagreement_values).flatten()
        result["reliability_environment_disagreement_mean"] = float(
        ...
        result["reliability_environment_disagreement_mean"] = float("nan")
        result["reliability_environment_disagreement_p90"] = float("nan")
    result["train/correction_cosine_mean"] = (
        float(torch.cat(correction_cosine_values).mean())
        if correction_cosine_values else float("nan")
    )
    result["train/correction_beneficial_ratio"] = (
        float(torch.cat(correction_beneficial_values).mean())
        if correction_beneficial_values else float("nan")
    )
    result["train/correction_magnitude_ratio_mean"] = (
        float(torch.cat(correction_magnitude_values).mean())
        if correction_magnitude_values else float("nan")
    )
    ...
    if fixed_reliability_sample_ids:
        fixed_ids = torch.cat(fixed_reliability_sample_ids)
        fixed_targets = torch.cat(fixed_reliability_targets)
        order = torch.argsort(fixed_ids)
        model._last_reliability_target_snapshot = (
            fixed_ids[order], fixed_targets[order]
        )
    else:
        model._last_reliability_target_snapshot = None
    return result
...
def _bounded_flatten(values, max_points=1_000_000):
    """Return a deterministic bounded view for diagnostic correlations."""
    values = torch.as_tensor(values).reshape(-1)
    if values.numel() > max_points:
        stride = (values.numel() + max_points - 1) // max_points
        values = values[::stride][:max_points]
    return values.to(torch.float64)


def pearson_correlation(left, right):
    left = _bounded_flatten(left)
    right = _bounded_flatten(right)
    finite = torch.isfinite(left) & torch.isfinite(right)
    left, right = left[finite], right[finite]
    if left.numel() < 2 or left.std() < 1e-12 or right.std() < 1e-12:
        return float("nan")
...
def _compact_prediction(prediction, width=256):
    """Deterministically sketch each sample for H-stability diagnostics."""
    flat = prediction.flatten(1)
    if flat.shape[1] <= width:
        return flat
    indices = torch.linspace(
        0, flat.shape[1] - 1, steps=width, device=flat.device
    ).round().long()
    return flat.index_select(1, indices)


def _paired_error_mean(left, right, absolute=False, chunk_size=1_000_000):
    """Memory-bounded paired MAE/MSE used only by CPU diagnostics."""
    left = torch.as_tensor(left).reshape(-1)
    right = torch.as_tensor(right).reshape(-1)
    if left.shape != right.shape:
        raise ValueError(f"diagnostic shape mismatch: {left.shape} vs {right.shape}")
    total = 0.0
    count = 0
    for start in range(0, left.numel(), chunk_size):
        delta = left[start : start + chunk_size].to(torch.float32) - right[
            start : start + chunk_size
        ].to(torch.float32)
        value = delta.abs() if absolute else delta.square()
        total += float(value.sum(dtype=torch.float64))
        count += value.numel()
    return total / max(count, 1)


def reliability_target_stability(snapshot, previous_epoch=None, previous_stage=None):
    """Summarize analytic reliability-target motion on fixed TRAIN samples."""
    result = {
        "r_target_epoch_correlation": float("nan"),
        "r_target_epoch_rank_stability": float("nan"),
        "r_target_stage_correlation": float("nan"),
        "r_target_stage_rank_stability": float("nan"),
        "r_target_std": float("nan"),
        "r_target_clip_zero_ratio": float("nan"),
        "r_target_clip_one_ratio": float("nan"),
        "r_target_fixed_sample_count": 0,
    }
    if snapshot is None:
        return result
    sample_id, target = snapshot
    target = torch.as_tensor(target, dtype=torch.float32)
    result.update({
        "r_target_std": float(target.std()),
        "r_target_clip_zero_ratio": float((target <= 1e-8).float().mean()),
        "r_target_clip_one_ratio": float((target >= 1.0 - 1e-8).float().mean()),
        "r_target_fixed_sample_count": int(target.shape[0]),
    })

    def compare(previous, prefix):
        if previous is None:
            return
        previous_id, previous_target = previous
        old = {int(index): row for index, row in zip(previous_id, previous_target)}
        current_rows, previous_rows = [], []
        for index, row in zip(sample_id, target):
            prior = old.get(int(index))
            if prior is not None and prior.shape == row.shape:
                current_rows.append(row)
                previous_rows.append(prior)
        if not current_rows:
            return
        current = torch.stack(current_rows)
        prior = torch.stack(previous_rows)
        result[f"r_target_{prefix}_correlation"] = pearson_correlation(
            current, prior
        )
        result[f"r_target_{prefix}_rank_stability"] = spearman_correlation(
            current, prior
        )

    compare(previous_epoch, "epoch")
    compare(previous_stage, "stage")
    return result


@torch.no_grad()
def build_pretrained_h_cache(model, ordered_train, args, device):
    predictions, norms, ids = [], [], []
    keep_full_prediction = args.lambda_h_anchor != 0
    model.eval()
    for x, _, sample_id, cycle_index in ordered_train:
        output = model.forward_components(
            x.float().to(device), cycle_index.to(device)
        )
        prediction = output["h_prediction"].cpu()
        if not keep_full_prediction:
        ...
        ids.append(sample_id)
    h_prediction = torch.cat(predictions)
    h_norm = torch.cat(norms)
    sample_id = torch.cat(ids)
...
def h_stability_diagnostics(
    ...
    comparison_prediction = h_prediction
    if comparison_prediction.shape[1:] != reference_prediction.shape[1:]:
        comparison_prediction = _compact_prediction(
            comparison_prediction, reference_prediction.shape[-1]
        )
    return {
        "h_prediction_MSE": _paired_error_mean(h_prediction, target),
        "h_prediction_MAE": _paired_error_mean(
            h_prediction, target, absolute=True
        ),
        "h_prediction_drift_from_pretrained": _paired_error_mean(
            comparison_prediction, reference_prediction
        ),
...
@torch.no_grad()
def correction_alignment_diagnostics(model, loader, args, device):
    """Measure raw gamma/beta correction direction and magnitude by patch."""
    empty = {
        "correction_cosine_mean": float("nan"),
        "correction_optimal_scalar_mean": float("nan"),
        "correction_beneficial_ratio": float("nan"),
        "correction_magnitude_ratio_mean": float("nan"),
        "correction_patch_cosine": [],
        "correction_patch_optimal_scalar": [],
        "correction_patch_beneficial_ratio": [],
        "correction_patch_magnitude_ratio": [],
    }
    if model.variant_fusion_mode != "horizon_future_var":
        return empty
    model.eval()
    cosine_rows, scalar_rows, beneficial_rows, magnitude_rows = [], [], [], []
    for step, batch in enumerate(loader):
        if args.max_eval_batches and step >= args.max_eval_batches:
            break
        x, y, _, cycle_index = batch[:4]
        x = x.float().to(device)
        y = y[:, -args.pred_len :].float().to(device)
        output = model.forward_components(x, cycle_index.to(device))
        gamma = output["horizon_gamma"].permute(0, 2, 1)
        beta = output["horizon_beta"].permute(0, 2, 1)
        raw_normalized = (
            (1.0 + gamma)
        ...
        raw_prediction = (
            raw_normalized * output["input_scale"] + output["input_shift"]
        )
        invariant = output["invariant_prediction"].detach()
        residual = y - invariant
        correction = raw_prediction - invariant
        batch_cosine, batch_scalar, batch_beneficial, batch_magnitude = [], [], [], []
        for start in range(0, args.pred_len, int(args.future_patch_len)):
            end = min(start + int(args.future_patch_len), args.pred_len)
            residual_patch = residual[:, start:end].flatten(1)
            correction_patch = correction[:, start:end].flatten(1)
            dot = (residual_patch * correction_patch).sum(1)
            residual_norm = residual_patch.square().sum(1).sqrt()
            correction_square = correction_patch.square().sum(1)
            correction_norm = correction_square.sqrt()
            batch_cosine.append(
                dot / (residual_norm * correction_norm).clamp_min(1e-8)
            )
            batch_scalar.append(dot / (correction_square + 1e-8))
            batch_beneficial.append((2.0 * dot > correction_square).float())
            batch_magnitude.append(
                correction_norm / residual_norm.clamp_min(1e-8)
            )
        cosine_rows.append(torch.stack(batch_cosine, 1).cpu())
        scalar_rows.append(torch.stack(batch_scalar, 1).cpu())
        beneficial_rows.append(torch.stack(batch_beneficial, 1).cpu())
        magnitude_rows.append(torch.stack(batch_magnitude, 1).cpu())
    if not cosine_rows:
        return empty
    cosine = torch.cat(cosine_rows)
    scalar = torch.cat(scalar_rows)
    beneficial = torch.cat(beneficial_rows)
    magnitude = torch.cat(magnitude_rows)
    result = {
        "correction_cosine_mean": float(cosine.mean()),
        "correction_optimal_scalar_mean": float(scalar.mean()),
        "correction_beneficial_ratio": float(beneficial.mean()),
        "correction_magnitude_ratio_mean": float(magnitude.mean()),
        "correction_patch_cosine": cosine.mean(0).tolist(),
        "correction_patch_optimal_scalar": scalar.mean(0).tolist(),
        "correction_patch_beneficial_ratio": beneficial.mean(0).tolist(),
        "correction_patch_magnitude_ratio": magnitude.mean(0).tolist(),
    }
    for name, values in (
        ("cosine", cosine),
        ("optimal_scalar", scalar),
        ("beneficial_ratio", beneficial),
        ("magnitude_ratio", magnitude),
    ):
        chunks = torch.tensor_split(values, min(3, values.shape[1]), dim=1)
        labels = ("near", "middle", "far")
        for label, chunk in zip(labels, chunks):
            result[f"correction_{name}_{label}"] = float(chunk.mean())
    return result
...
def evaluate(
    ...
    analytic_reliability_usefulness = []
    environment_disagreement_values = []
    ...
    for step, (x, y, _, cycle_index) in enumerate(test_loader):
        ...
        if output["horizon_reliability"] is not None:
            ...
            usefulness_target_h = patch_correction_usefulness_target(
                y,
                output["invariant_prediction"],
                raw_prediction,
                args.future_patch_len,
            ...
            analytic_reliability_usefulness.append(
                usefulness_target_h.cpu()
            )
    ...
    result = {
        ...
        "lambda_variant_anchor": args.lambda_variant_anchor,
        ...
        "loss/variant_total": (
            raw_mse + args.lambda_variant_anchor * raw_anchor_loss_value
            if horizon_future_enabled
            else float("nan")
        ),
        "variant/lambda_maturity": maturity_snapshot["maturity"],
        "maturity": maturity_snapshot["maturity"],
        "G_inv_L2": maturity_snapshot["G_inv_L2"],
        "G_var_L2": maturity_snapshot["G_var_L2"],
        "G_inv": maturity_snapshot["G_inv_L2"],
        "G_var": maturity_snapshot["G_var_L2"],
    ...
    if analytic_reliability_predictions:
        ...
        usefulness_target = torch.cat(
            analytic_reliability_usefulness, dim=0
        )
        ...
        result["reliability_objective"] = args.reliability_objective
        result["reliability_fraction_mse"] = float(
            F.mse_loss(r_prediction, r_target)
        )
        result["reliability_ranking_loss"] = float(
            reliability_pairwise_ranking_loss(r_prediction, r_target)
        )
        result["reliability_usefulness_bce"] = float(
            F.binary_cross_entropy(
                r_prediction.clamp(1e-6, 1.0 - 1e-6), usefulness_target
            )
        )
        result["reliability_usefulness_accuracy"] = float(
            ((r_prediction >= 0.5) == (usefulness_target >= 0.5))
            .float().mean()
        )
        result["reliability_usefulness_positive_ratio"] = float(
            usefulness_target.mean()
        )
        for index, chunk in enumerate(
...
def parse_args():
    ...
    parser.add_argument("--adaptive_variant_lr", action="store_true")
    parser.add_argument(
        "--fixed_gamma_beta_lr",
        action="store_true",
        help="Keep gamma/beta at lr_gamma_beta_base instead of maturity scaling.",
    )
    ...
    parser.add_argument(
        ...
        choices=("mse", "mse_ranking", "mse_usefulness"),
        default="mse",
    )
    parser.add_argument(
        "--reliability_environment_disagreement",
        action="store_true",
        help=(
            "Train isolated environment-specific correction estimates and "
            "append their disagreement to the reliability head."
        ),
    )
    ...
    parser.add_argument("--lambda_variant_anchor", type=float, default=1.0)
    parser.add_argument(
    ...
    parser.add_argument(
        "--horizon_reliability_gate",
        choices=("on", "off"),
        default="on",
        help=(
            "Apply predicted patch reliability to Future-Zvar correction "
            "during validation/test. Training always learns the raw correction."
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        choices=("predictive", "random", "shuffled", "uniform"),
        default="predictive",
        help="Causal control applied after TRAIN-only predictive environment discovery.",
    )
...
def main():
    ...
    if (
        ...
        or args.lambda_variant_anchor < 0
    ):
    ...
    for experiment in experiments:
        ...
        epoch, stage = 0, 0
        previous_reliability_snapshot = None
        previous_stage_reliability_snapshot = None
        while epoch < args.epochs:
            if manager is not None:
                ...
                assignment = controlled_environment_assignment(
                    assignment,
                    args.environment_supervision,
                    args.seed,
                    stage,
                )
                record["environment_supervision"] = args.environment_supervision
                record["supervision_environment_mass"] = (
                    assignment.mean(axis=0).tolist()
                )
                record.update(
            ...
            for _ in range(epochs_this_stage):
                ...
                current_reliability_snapshot = getattr(
                    model, "_last_reliability_target_snapshot", None
                )
                epoch_record.update(reliability_target_stability(
                    current_reliability_snapshot,
                    previous_reliability_snapshot,
                    previous_stage_reliability_snapshot,
                ))
                previous_reliability_snapshot = current_reliability_snapshot
        ...
        for split_name, diagnostic_loader in (
            ("train", ordered_train),
            ("test", test_loader),
        ):
            correction_metrics = correction_alignment_diagnostics(
                model, diagnostic_loader, args, device
            )
            result.update({
                f"{split_name}/{key}": value
                for key, value in correction_metrics.items()
            })
        (
            _,
            final_h_target,
        ...
        result.update(
            {
                ...
                "adaptive_variant_lr": args.adaptive_variant_lr,
                "environment_supervision": args.environment_supervision,
            }
        )
        if training_history:
            ...
            for key in (
                ...
                "G_inv_L2",
                "G_var_L2",
                "r_target_epoch_correlation",
                "r_target_epoch_rank_stability",
                "r_target_stage_correlation",
                "r_target_stage_rank_stability",
                "r_target_std",
                "r_target_clip_zero_ratio",
                "r_target_clip_one_ratio",
                "r_target_fixed_sample_count",
            ):
        ...
        for split_name in ("train", "test"):
            correction_lines.append(f"[{split_name}]")
            prefix = f"{split_name}/correction_"
            for key in sorted(result):
                if key.startswith(prefix):
                    correction_lines.append(f"{key}: {result[key]}")
        (experiment_dir / "correction_diagnostics.txt").write_text(
