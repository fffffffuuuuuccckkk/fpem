...
from tools.run_predictive_env_iv_patchtst import (
    build_data,
    controlled_environment_assignment,
    model_config,
)
...
def environment_for_visualization(environment, run_args, label_mode, stage=-1):
    """Return the TRAIN labels used for coloring without changing features."""
    result = dict(environment)
    q = np.asarray(environment["q"], dtype=np.float32)
    used_stage = int(stage)
    if used_stage < 0:
        used_stage = max(
            (int(run_args.epochs) - 1) // int(run_args.stage_epochs), 0
        )
    supervision = "predictive"
    if label_mode == "training_supervision":
        supervision = str(
            getattr(run_args, "environment_supervision", "predictive")
        )
        q = controlled_environment_assignment(
            q, supervision, int(run_args.seed), used_stage
        )
    result["q"] = q
    result["hard_env"] = q.argmax(1).astype(np.int64)
    return result, supervision, used_stage
...
def main():
    ...
    parser.add_argument(
        "--label_mode",
        choices=("predictive", "training_supervision"),
        default="training_supervision",
        help=(
            "Color by raw EIIL environments or by the detached environment "
            "supervision actually used to train the checkpoint."
        ),
    )
    parser.add_argument(
        "--supervision_stage",
        type=int,
        default=-1,
        help="Environment update stage for deterministic shuffled labels; -1 uses the final stage.",
    )
    ...
    environment, supervision, supervision_stage = environment_for_visualization(
        environment,
        run_args,
        args.label_mode,
        args.supervision_stage,
    )
    ...
    summary = {
        ...
        "environment_supervision": supervision,
