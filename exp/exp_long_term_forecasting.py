...
class Exp_Long_Term_Forecast(Exp_Basic):
    ...
    def test(self, setting, test=0):
        ...
        diagnostic_values = {
            ...
            "raw_future_probability": [], "raw_full_future_probability": [],
            "raw_future_top1": [],
            "raw_future_top3_mass": [], "raw_mapping_gate": [],
            "raw_shape_gate": [], "raw_scale_gate": [], "raw_shift_gate": [],
            "raw_shape_residual_norm": [], "raw_predicted_scale": [],
            "raw_predicted_shift": [],
            "oracle_top1_invariant": [], "oracle_top1_final": [],
            "oracle_top3_invariant": [], "oracle_top3_final": [],
        }
        baseline_model = None
        baseline_preds = []
            ...
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(test_loader):
                ...
                if is_pmg:
                    ...
                    if getattr(module, "future_mapping_mode", "off") != "off":
                        ...
                        if getattr(module, "fpem_forecast_mode", "hidden_correction") == "raw_pattern":
                            raw_probability = latest["raw_future_probability"]
                            raw_full_probability = latest["raw_full_future_probability"]
                            diagnostic_values["raw_full_future_probability"].append(
                                raw_full_probability.detach().cpu().numpy()
                            )
                            top_count = min(3, raw_full_probability.shape[-1])
                            diagnostic_values["raw_future_probability"].append(raw_probability.detach().cpu().numpy())
                            diagnostic_values["raw_future_top1"].append(raw_probability.argmax(-1).detach().cpu().numpy())
                            diagnostic_values["raw_future_top3_mass"].append(
                                raw_full_probability.topk(top_count, dim=-1).values.sum(-1).detach().cpu().numpy()
                            )
                            diagnostic_values["raw_mapping_gate"].append(
                                latest["raw_mapping_gate"].detach().cpu().numpy()
                            )
                            for diagnostic_name in (
                                "raw_shape_gate", "raw_scale_gate", "raw_shift_gate",
                                "raw_predicted_scale", "raw_predicted_shift",
                            ):
                                tensor = latest[diagnostic_name]
                                if tensor.ndim == 4 and tensor.shape[-1] == 1:
                                    tensor = tensor.squeeze(-1)
                                diagnostic_values[diagnostic_name].append(tensor.detach().cpu().numpy())
                            diagnostic_values["raw_shape_residual_norm"].append(
                                latest["raw_shape_delta"].norm(dim=-1).detach().cpu().numpy()
                            )
                    ...
                    if getattr(module, "fpem_forecast_mode", "hidden_correction") == "raw_pattern":
                        oracle = module.fpem_raw_oracle_diagnostics(
                            batch_y[:, -self.args.pred_len:, :]
                        )
                        oracle_predictions = {
                            name: oracle[name].detach().cpu().numpy()
                            for name in (
                                "oracle_top1_invariant_forecast", "oracle_top1_final_forecast",
                                "oracle_top3_invariant_forecast", "oracle_top3_final_forecast",
                            )
                        }
                ...
                if test_data.scale and self.args.inverse:
                    ...
                    if prediction_inv is not None:
                        ...
                        if oracle_predictions is not None:
                            oracle_predictions = {
                                name: test_data.inverse_transform(
                                    value.reshape(shape[0] * shape[1], -1)
                                ).reshape(shape)
                                for name, value in oracle_predictions.items()
                            }
                ...
                if prediction_inv is not None:
                    ...
                    if oracle_predictions is not None:
                        for source_name, destination_name in (
                            ("oracle_top1_invariant_forecast", "oracle_top1_invariant"),
                            ("oracle_top1_final_forecast", "oracle_top1_final"),
                            ("oracle_top3_invariant_forecast", "oracle_top3_invariant"),
                            ("oracle_top3_final_forecast", "oracle_top3_final"),
                        ):
                            diagnostic_values[destination_name].append(
                                oracle_predictions[source_name][:, :, f_dim:]
                            )
                if baseline_output is not None:
        ...
        if is_pmg and diagnostic_values["c_inv"]:
            ...
            with open(diagnostics_path, "w", encoding="utf-8") as handle:
                ...
                handle.write("forecast/mode: {}\n".format(
                    getattr(module, "fpem_forecast_mode", "hidden_correction")
                ))
                ...
                train_window_count = int(module._graph_metadata.get("train_window_count", 0))
                anchor_count = max(1, int(np.ceil(train_window_count / float(module.seq_len))))
                pattern_anchor_coverage = (
                    pattern_support.detach().cpu().numpy() / float(anchor_count)
                    if pattern_support.numel() else np.asarray([])
                )
                if pattern_anchor_coverage.size:
                    handle.write("pattern/sample_coverage_mean: {:.10g}\n".format(
                        float(pattern_anchor_coverage.mean())
                    ))
                    handle.write("pattern/sample_coverage_median: {:.10g}\n".format(
                        float(np.median(pattern_anchor_coverage))
                    ))
                    for qname, fraction in (("p10", .1), ("p50", .5), ("p90", .9)):
                        handle.write("pattern/sample_coverage_{}: {:.10g}\n".format(
                            qname, float(np.quantile(pattern_anchor_coverage, fraction))
                        ))
                ...
                if coverage.size:
                    stable_coverage = float(np.sum(coverage * p_inv) / max(np.sum(p_inv), 1e-12))
                    handle.write("mapping/stable_coverage_mean: {:.10g}\n".format(stable_coverage))
                ...
                if module.future_mapping_mode != "off":
                    ...
                    if getattr(module, "fpem_forecast_mode", "hidden_correction") == "raw_pattern":
                        raw_probability = np.concatenate(diagnostic_values["raw_future_probability"])
                        raw_full_probability = np.concatenate(
                            diagnostic_values["raw_full_future_probability"]
                        )
                        raw_top1 = np.concatenate(diagnostic_values["raw_future_top1"])
                        raw_top3_mass = np.concatenate(diagnostic_values["raw_future_top3_mass"])
                        raw_mapping_gate = np.concatenate(diagnostic_values["raw_mapping_gate"])
                        raw_shape_gate = np.concatenate(diagnostic_values["raw_shape_gate"])
                        raw_scale_gate = np.concatenate(diagnostic_values["raw_scale_gate"])
                        raw_shift_gate = np.concatenate(diagnostic_values["raw_shift_gate"])
                        raw_shape_norm = np.concatenate(diagnostic_values["raw_shape_residual_norm"])
                        raw_predicted_scale = np.concatenate(diagnostic_values["raw_predicted_scale"])
                        raw_predicted_shift = np.concatenate(diagnostic_values["raw_predicted_shift"])
                        handle.write("raw_forecast/mapping_mode: {}\n".format(module.raw_mapping_mode))
                        handle.write("raw_forecast/shape_mode: {}\n".format(module.raw_shape_mode))
                        handle.write("raw_forecast/scale_mode: {}\n".format(module.raw_scale_mode))
                        handle.write("raw_forecast/shift_mode: {}\n".format(module.raw_shift_mode))
                        handle.write("raw_forecast/decoder_context: {}\n".format(module.raw_decoder_context))
                        handle.write("raw_forecast/pattern_selection: {}\n".format(module.pattern_selection))
                        handle.write("raw_forecast/pattern_topk: {}\n".format(module.pattern_topk))
                        handle.write("raw_forecast/pattern_temperature: {:.10g}\n".format(module.pattern_temperature))
                        handle.write("raw_forecast/pattern_straight_through: {}\n".format(
                            str(module.pattern_straight_through).lower()
                        ))
                        selected_entropy = -(
                            raw_probability * np.log(np.maximum(raw_probability, 1e-12))
                        ).sum(-1)
                        full_entropy = -(
                            raw_full_probability * np.log(np.maximum(raw_full_probability, 1e-12))
                        ).sum(-1)
                        top1_probability = raw_full_probability.max(-1)
                        top3_probability = np.sort(raw_full_probability, axis=-1)[..., -min(3, raw_full_probability.shape[-1]):].sum(-1)
                        top5_probability = np.sort(raw_full_probability, axis=-1)[..., -min(5, raw_full_probability.shape[-1]):].sum(-1)
                        effective_patterns = np.exp(selected_entropy)
                        for name, array in (
                            ("mapping_gate", raw_mapping_gate),
                            ("full_probability_entropy", full_entropy),
                            ("selected_probability_entropy", selected_entropy),
                            ("top1_probability", top1_probability),
                            ("top3_cumulative_probability", top3_probability),
                            ("top5_cumulative_probability", top5_probability),
                            ("effective_patterns", effective_patterns),
                            ("shape_gate", raw_shape_gate),
                            ("scale_gate", raw_scale_gate),
                            ("shift_gate", raw_shift_gate),
                            ("shape_residual_norm", raw_shape_norm),
                            ("predicted_scale", raw_predicted_scale),
                            ("predicted_shift", raw_predicted_shift),
                        ):
                            handle.write("raw_forecast/{}_mean: {:.10g}\n".format(name, float(array.mean())))
                            handle.write("raw_forecast/{}_std: {:.10g}\n".format(name, float(array.std())))
                            for qname, fraction in (("p10", .1), ("p50", .5), ("p90", .9)):
                                handle.write("raw_forecast/{}_{}: {:.10g}\n".format(
                                    name, qname, float(np.quantile(array, fraction))
                                ))
                        # Keep channel and future-patch axes intact: no flattened temporal neighbours.
                        for future_index in range(raw_top1.shape[2]):
                            counts = np.bincount(
                                raw_top1[:, :, future_index].reshape(-1),
                                minlength=raw_probability.shape[-1],
                            )
                            handle.write("raw_forecast/horizon_patch_{}/top1_patterns: {}\n".format(
                                future_index, ",".join(map(str, counts.tolist()))
                            ))
                            nonzero = counts[counts > 0]
                            usage_probability = nonzero / max(nonzero.sum(), 1)
                            usage_entropy = -float(np.sum(
                                usage_probability * np.log(np.maximum(usage_probability, 1e-12))
                            ))
                            handle.write("raw_forecast/horizon_patch_{}/unique_patterns: {}\n".format(
                                future_index, int(nonzero.size)
                            ))
                            handle.write("raw_forecast/horizon_patch_{}/dominant_pattern_ratio: {:.10g}\n".format(
                                future_index, float(counts.max() / max(counts.sum(), 1))
                            ))
                            handle.write("raw_forecast/horizon_patch_{}/pattern_usage_entropy: {:.10g}\n".format(
                                future_index, usage_entropy
                            ))
                            handle.write("raw_forecast/horizon_patch_{}/top3_mass: {:.10g}\n".format(
                                future_index, float(raw_top3_mass[:, :, future_index].mean())
                            ))
                            handle.write("raw_forecast/horizon_patch_{}/shape_activation: {:.10g}\n".format(
                                future_index, float(raw_shape_gate[:, :, future_index].mean())
                            ))
                            handle.write("raw_forecast/horizon_patch_{}/scale_activation: {:.10g}\n".format(
                                future_index, float(raw_scale_gate[:, :, future_index].mean())
                            ))
                            handle.write("raw_forecast/horizon_patch_{}/shift_activation: {:.10g}\n".format(
                                future_index, float(raw_shift_gate[:, :, future_index].mean())
                            ))
                        for stage_name, stage_prediction in (
                            ("invariant", prediction_inv), ("shape", prediction_shape),
                            ("geometry", prediction_scale), ("final", prediction_typed),
                        ):
                            handle.write("raw_forecast/{}_reconstruction_mse: {:.10g}\n".format(
                                stage_name, float(np.mean((stage_prediction - trues) ** 2))
                            ))
                        gate_zero_difference = (
                            float(np.max(np.abs(prediction_typed - prediction_inv)))
                            if module.raw_mapping_mode == "stable_only"
                            and all(mode == "off" for mode in (
                                module.raw_shape_mode, module.raw_scale_mode, module.raw_shift_mode
                            )) else float("nan")
                        )
                        handle.write("raw_forecast/gate_zero_max_difference: {:.10g}\n".format(
                            gate_zero_difference
                        ))
                        oracle_top1_invariant = np.concatenate(diagnostic_values["oracle_top1_invariant"])
                        oracle_top1_final = np.concatenate(diagnostic_values["oracle_top1_final"])
                        oracle_top3_invariant = np.concatenate(diagnostic_values["oracle_top3_invariant"])
                        oracle_top3_final = np.concatenate(diagnostic_values["oracle_top3_final"])
                        # These are dictionary-capacity diagnostics only and are deliberately
                        # excluded from metrics.txt and all checkpoint/model-selection logic.
                        for oracle_name, oracle_prediction in (
                            ("top1_invariant", oracle_top1_invariant),
                            ("top1_final", oracle_top1_final),
                            ("top3_invariant", oracle_top3_invariant),
                            ("top3_final", oracle_top3_final),
                        ):
                            handle.write("oracle/{}/mse: {:.10g}\n".format(
                                oracle_name, float(np.mean((oracle_prediction - trues) ** 2))
                            ))
                        handle.write("oracle/diagnostic_only: true\n")
                        handle.write("oracle/used_for_training_or_selection: false\n")
                        np.savez(
                            os.path.join(folder_path, "raw_pattern_diagnostics.npz"),
                            responsibility=raw_probability,
                            full_probability=raw_full_probability,
                            top1_pattern=raw_top1,
                            top3_mass=raw_top3_mass,
                            mapping_gate=raw_mapping_gate,
                            shape_gate=raw_shape_gate,
                            scale_gate=raw_scale_gate,
                            shift_gate=raw_shift_gate,
                            ...
                            predicted_scale=raw_predicted_scale,
                            predicted_shift=raw_predicted_shift,
                            invariant_forecast=prediction_inv,
                            shape_forecast=prediction_shape,
                            geometry_forecast=prediction_scale,
                            final_forecast=prediction_typed,
                            oracle_top1_invariant_forecast=oracle_top1_invariant,
                            oracle_top1_final_forecast=oracle_top1_final,
                            oracle_top3_invariant_forecast=oracle_top3_invariant,
                            oracle_top3_final_forecast=oracle_top3_final,
                        ...
                        sample_final_mse = ((prediction_typed - trues) ** 2).mean(axis=(1, 2))
                        sample_stable_mse = ((base_future_prediction - trues) ** 2).mean(axis=(1, 2))
                        with open(os.path.join(folder_path, "raw_pattern_samples.txt"), "w", encoding="utf-8") as sample_file:
                            sample_file.write(
                                "sample\tstable_mse\tfinal_mse\tgain\tmapping_gate\tshape_gate\t"
                                "scale_gate\tshift_gate\tpattern_entropy\n"
                            )
                            sample_arrays = (
                                raw_mapping_gate.reshape(raw_mapping_gate.shape[0], -1).mean(1),
                                raw_shape_gate.reshape(raw_shape_gate.shape[0], -1).mean(1),
                                raw_scale_gate.reshape(raw_scale_gate.shape[0], -1).mean(1),
                                raw_shift_gate.reshape(raw_shift_gate.shape[0], -1).mean(1),
                                selected_entropy.reshape(selected_entropy.shape[0], -1).mean(1),
                            )
                            for sample_index in range(sample_final_mse.shape[0]):
                                sample_file.write(
                                    "{}\t{:.10g}\t{:.10g}\t{:.10g}\t{:.10g}\t{:.10g}\t{:.10g}\t{:.10g}\t{:.10g}\n".format(
                                        sample_index, sample_stable_mse[sample_index],
                                        sample_final_mse[sample_index],
                                        sample_stable_mse[sample_index] - sample_final_mse[sample_index],
                                        *(array[sample_index] for array in sample_arrays)
                                    )
                                )
