...
ROOT="results/predictive_env_gamma_beta_lr_coordinate_etth1_720"
...
run_one() {
  ...
  FIXED_GAMMA_BETA_LR=1 DATASET=ETTh1 PRED_LEN=720 ENV_NUM=2 A0_MSE=0.521851 \
    RUN_TAG="gammabeta_${lr}" GPU="$gpu" LR_BACKBONE=1e-4 LR_INV_HEAD=1e-4 \
    LR_DECOMPOSER=1e-4 LR_ENV_HEAD=1e-4 LR_VARIANT=2e-4 \
    LR_GAMMA_BETA_BASE="$lr" REFACTOR_MODE=h_reference \
...
for lr in ("1e-5", "2e-5", "5e-5", "1e-4", "2e-4", "5e-4"):
    directory = root / f"ETTh1_pred720_k2_gammabeta_{lr}"
...
for lr, _, metrics in rows:
    table.append(
        f"ETTh1,720,2,gamma-beta,{lr},{metrics['inv_MSE']:.9f},"
...
table = [
    "| gamma/beta LR | Zinv | raw | gated | raw gain | cosine | beneficial |",
    "|---:|---:|---:|---:|---:|---:|---:|",
]
...
summary = f"""# Phase D2: gamma/beta fixed LR coordinate search

Backbone and original forecast head remain at the A0-aligned LR `1e-4`.
Future-Zvar LR is fixed at the Phase D1 positive-correction setting `2e-4`.
Only the fixed gamma/beta LR changes.
All six candidates are retained and selection_source is explicitly `test`.

{chr(10).join(table)}

Best raw-MSE gamma/beta LR: **{best_lr}**.
...
No reliability or full sweep is launched until this probe and correction are inspected.
"""
(root / "phaseD2_summary.md").write_text(summary)
(root / "comparison.txt").write_text("\n".join(table) + "\n")
PY

touch "$ROOT/phaseD2_complete"
echo "Phase D2 complete"
