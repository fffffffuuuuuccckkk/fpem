...
OUTPUT_ROOT="${OUTPUT_ROOT:-results/predictive_env_future_var_k3}"
REFERENCE_CHECKPOINT="${REFERENCE_CHECKPOINT:-results/predictive_env_decomposition_comparison_k3/shared_reference.pt}"
PREVIOUS_ROOT="${PREVIOUS_ROOT:-results/predictive_env_complementary_fusion_k3}"
...
for lambda_future_var in 0 0.01 0.05 0.1 0.2; do
  lambda_tag="${lambda_future_var/./p}"
  echo "future-Zvar A2 K=3 lambda_future_var=$lambda_future_var"
  ...
  LAMBDA_INVPRED=1.0 \
  LAMBDA_FUTURE_VAR="$lambda_future_var" \
...
"$PYTHON" tools/summarize_predictive_env_future_var_sweep.py \
  "$OUTPUT_ROOT" --previous_root "$PREVIOUS_ROOT"
cat "$OUTPUT_ROOT/future_var_summary.txt"
