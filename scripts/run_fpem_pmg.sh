...
FORECAST_MODE="${FORECAST_MODE:-hidden_correction}"
RAW_MAPPING_MODE="${RAW_MAPPING_MODE:-stable_only}"
RAW_SHAPE_MODE="${RAW_SHAPE_MODE:-gated}"
RAW_SCALE_MODE="${RAW_SCALE_MODE:-gated}"
RAW_SHIFT_MODE="${RAW_SHIFT_MODE:-gated}"
...
RAW_EXPERIMENT="${RAW_EXPERIMENT:-custom}"
...
COMMON_FLAGS=(
  ...
  --fpem_forecast_mode "$FORECAST_MODE"
  --fpem_raw_mapping_mode "$RAW_MAPPING_MODE"
  --fpem_raw_shape_mode "$RAW_SHAPE_MODE"
  --fpem_raw_scale_mode "$RAW_SCALE_MODE"
  --fpem_raw_shift_mode "$RAW_SHIFT_MODE"
  --fpem_raw_decoder_context "$RAW_DECODER_CONTEXT"
  --fpem_pattern_selection "$PATTERN_SELECTION"
  --fpem_pattern_topk "$PATTERN_TOPK"
  --fpem_pattern_temperature "$PATTERN_TEMPERATURE"
  --fpem_pattern_straight_through "$PATTERN_STRAIGHT_THROUGH"
)
...
if [[ "$SMOKE" == "1" ]]; then
  ...
  if [[ "$FORECAST_MODE" == "raw_pattern" ]]; then
    DESCRIPTION="fpem_raw_${RAW_EXPERIMENT}_sel${PATTERN_SELECTION}k${PATTERN_TOPK}_${RAW_MAPPING_MODE}_sh${RAW_SHAPE_MODE}_sc${RAW_SCALE_MODE}_st${RAW_SHIFT_MODE}_${RAW_DECODER_CONTEXT}_smoke"
  else
  ...
  [[ "$PMG_SPACE" == "raw" ]] && SPACE_TAG="raw"
  if [[ "$FORECAST_MODE" == "raw_pattern" ]]; then
    DESCRIPTION="fpem_raw_${RAW_EXPERIMENT}_sel${PATTERN_SELECTION}k${PATTERN_TOPK}_${RAW_MAPPING_MODE}_sh${RAW_SHAPE_MODE}_sc${RAW_SCALE_MODE}_st${RAW_SHIFT_MODE}_${RAW_DECODER_CONTEXT}"
  else
