...
from models.fpem import (
    ...
    RawPatternForecaster, StableFutureMappingMemory, StablePatternGraph,
)
...
class Model(nn.Module):
    ...
    def _init_pmg(self, configs: Any) -> None:
        ...
        self.fpem_forecast_mode = str(
            getattr(configs, "fpem_forecast_mode", "hidden_correction")
        ).lower()
        if self.fpem_forecast_mode not in {"hidden_correction", "raw_pattern"}:
            raise ValueError("fpem_forecast_mode must be hidden_correction or raw_pattern")
        self.raw_mapping_mode = str(getattr(configs, "fpem_raw_mapping_mode", "stable_only")).lower()
        self.raw_shape_mode = str(getattr(configs, "fpem_raw_shape_mode", "gated")).lower()
        self.raw_scale_mode = str(getattr(configs, "fpem_raw_scale_mode", "gated")).lower()
        self.raw_shift_mode = str(getattr(configs, "fpem_raw_shift_mode", "gated")).lower()
        self.raw_decoder_context = str(
            getattr(configs, "fpem_raw_decoder_context", "pattern_plus_hidden")
        ).lower()
        self.pattern_selection = str(
            getattr(configs, "fpem_pattern_selection", "soft")
        ).lower()
        self.pattern_topk = int(getattr(configs, "fpem_pattern_topk", 3))
        self.pattern_temperature = float(getattr(configs, "fpem_pattern_temperature", 1.0))
        self.pattern_straight_through = _flag(configs, "fpem_pattern_straight_through", 1)
        self.pattern_bank_mode = str(getattr(configs, "fpem_pattern_bank_mode", "legacy")).lower()
        self.shape_basis_rank = int(getattr(configs, "fpem_shape_basis_rank", 2))
        self.invariant_consolidation = _flag(configs, "fpem_invariant_consolidation", 1)
        self.local_pattern_merge = _flag(configs, "fpem_local_pattern_merge", 1)
        if any(mode not in {"off", "unit", "gated"} for mode in (
            self.raw_shape_mode, self.raw_scale_mode, self.raw_shift_mode
        )):
            raise ValueError("raw shape/scale/shift modes must be off, unit, or gated")
        if self.raw_decoder_context not in {"pattern_only", "pattern_plus_hidden"}:
            raise ValueError("invalid raw decoder context: {}".format(self.raw_decoder_context))
        if self.pattern_selection not in {"soft", "top1", "topk"}:
            raise ValueError("fpem_pattern_selection must be soft, top1, or topk")
        if self.pattern_topk < 1:
            raise ValueError("fpem_pattern_topk must be at least one")
        if self.pattern_temperature <= 0:
            raise ValueError("fpem_pattern_temperature must be positive")
        if self.pattern_bank_mode not in {"legacy", "deformable"}:
            raise ValueError("fpem_pattern_bank_mode must be legacy or deformable")
        if self.shape_basis_rank not in {0, 1, 2, 4}:
            raise ValueError("fpem_shape_basis_rank must be 0, 1, 2, or 4")
        if self.fpem_forecast_mode == "raw_pattern":
            if self.representation_space != "raw":
                raise ValueError("raw_pattern forecast requires representation_space=raw")
            mapping_to_future = {
                "stable_only": "stable_only", "variant_unit": "adaptive_unit",
                "variant_gated": "adaptive_gated",
            }
            if self.raw_mapping_mode not in mapping_to_future:
                raise ValueError("invalid raw mapping mode: {}".format(self.raw_mapping_mode))
            self.future_mapping_mode = mapping_to_future[self.raw_mapping_mode]
        ...
        print("FPem forecast mode: {}".format(self.fpem_forecast_mode))
        if self.fpem_forecast_mode == "raw_pattern":
            print("Raw pattern modes: mapping={} shape={} scale={} shift={} context={}".format(
                self.raw_mapping_mode, self.raw_shape_mode, self.raw_scale_mode,
                self.raw_shift_mode, self.raw_decoder_context,
            ))
            print("Future pattern selection: mode={} topk={} temperature={} straight_through={}".format(
                self.pattern_selection, self.pattern_topk, self.pattern_temperature,
                self.pattern_straight_through,
            ))
            print("Pattern bank mode: {} (rank={}, consolidation={}, local_merge={})".format(
                self.pattern_bank_mode, self.shape_basis_rank,
                self.invariant_consolidation, self.local_pattern_merge,
            ))
        ...
        self.raw_pattern_forecaster = None
    ...
    def _initialize_future_mapping_modules(self, history_feature_dim: int,
        ...
        if self.fpem_forecast_mode == "raw_pattern":
            self.raw_pattern_forecaster = RawPatternForecaster(
                self.d_model, self.patch_len, future_patch_count,
                self.pred_len, self.stride, self.raw_decoder_context,
            ).to(device)
    ...
    def _future_pattern_selection(self, decomposition: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
    ...
    def _apply_future_mapping(self, decomposition: Dict[str, torch.Tensor],
        ...
        result = self._future_pattern_selection(decomposition)
        probability = result["future_pattern_probability"]
        ...
        correction = self.future_forecast_correction(
            ...
            result["retrieval_confidence"], result["future_variant_gate"],
        )
    ...
    def _pmg_forecast(self, x_enc: torch.Tensor) -> torch.Tensor:
        ...
        if self.fpem_forecast_mode == "raw_pattern":
            future_mapping = self._future_pattern_selection(decomposition)
            raw_forecast = self.raw_pattern_forecaster(
                future_mapping["stable_future_logits"], future_mapping["variant_future_logits"],
                future_mapping["future_variant_gate"],
                self.future_pattern_graph.means.to(hidden.dtype),
                self.future_pattern_graph.stable_level_center.to(hidden.dtype),
                self.future_pattern_graph.stable_log_scale_center.exp().to(hidden.dtype),
                future_mapping["history_context"], decomposition["z_relation"].mean(2),
                self.raw_mapping_mode, self.raw_shape_mode,
                self.raw_scale_mode, self.raw_shift_mode,
                self.pattern_selection, self.pattern_topk,
                self.pattern_temperature, self.pattern_straight_through,
            )
            future_mapping.update(raw_forecast)
            y_inv = raw_forecast["raw_invariant_forecast"]
            y_shape = raw_forecast["raw_after_shape_forecast"]
            y_scale = raw_forecast["raw_after_geometry_forecast"]
            y_shift = y_scale
            y_typed = raw_forecast["raw_final_forecast"]
            y_full = y_typed
            y_base = raw_forecast["raw_without_variant_forecast"]
            y_env = y_inv
            y_without_var_map = raw_forecast["raw_without_variant_forecast"]
            y_with_var_map = y_full
        else:
    ...
    @torch.no_grad()
    def fpem_raw_oracle_diagnostics(self, target: torch.Tensor) -> Dict[str, torch.Tensor]:
        """Evaluation-only dictionary-capacity diagnostic; never used by forward/loss."""
        if self.training:
            raise RuntimeError("raw pattern oracle is diagnostics-only and requires eval mode")
        if self.fpem_forecast_mode != "raw_pattern" or not self.latest_fpem:
            raise RuntimeError("raw pattern oracle requires a completed raw-pattern forward pass")
        raw_target = extract_raw_patches(
            target[:, -self.pred_len:, :], self.patch_len, self.stride, self.stride
        )["normalized"]
        prototypes = self.future_pattern_graph.means.to(raw_target.dtype)
        distance = (raw_target.unsqueeze(-2) - prototypes.view(1, 1, 1, *prototypes.shape)).square().mean(-1)
        top1_index = distance.argmin(-1)
        top1 = F.one_hot(top1_index, prototypes.shape[0]).to(raw_target.dtype)
        keep = min(3, prototypes.shape[0])
        top3_distance, top3_index = distance.topk(keep, dim=-1, largest=False)
        top3_weight = torch.softmax(-top3_distance, -1)
        top3 = torch.zeros_like(distance).scatter(-1, top3_index, top3_weight)
        values = self.latest_fpem
        common = (
            prototypes,
            self.future_pattern_graph.stable_level_center.to(raw_target.dtype),
            self.future_pattern_graph.stable_log_scale_center.exp().to(raw_target.dtype),
            values["history_context"], values["z_relation"].mean(2),
            self.raw_shape_mode, self.raw_scale_mode, self.raw_shift_mode,
        )
        oracle_top1 = self.raw_pattern_forecaster.forecast_selected_probability(top1, *common)
        oracle_top3 = self.raw_pattern_forecaster.forecast_selected_probability(top3, *common)
        return {
            "oracle_top1_probability": top1,
            "oracle_top3_probability": top3,
            "oracle_top1_invariant_forecast": oracle_top1["raw_invariant_forecast"],
            "oracle_top1_final_forecast": oracle_top1["raw_final_forecast"],
            "oracle_top3_invariant_forecast": oracle_top3["raw_invariant_forecast"],
            "oracle_top3_final_forecast": oracle_top3["raw_final_forecast"],
        }
    ...
    def fpem_extra_loss(self, target: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, float]]:
        ...
        if self.fpem_forecast_mode == "raw_pattern":
            extra = self.lambda_future_map * future_map_loss
        else:
            extra = (
        ...
        if self.future_mapping_mode != "off":
            ...
            if self.fpem_forecast_mode == "hidden_correction":
                logs.update({
                    "future_mapping/forecast_gate_mean": float(values["future_forecast_gate"].mean().detach().cpu()),
                    "future_mapping/forecast_correction_norm": float(
                        values["future_forecast_correction"].float().norm(dim=1).mean().detach().cpu()
                    ),
                })
        if self.fpem_forecast_mode == "raw_pattern":
            shape_gate = values["raw_shape_gate"].squeeze(-1)
            scale_gate = values["raw_scale_gate"].squeeze(-1)
            shift_gate = values["raw_shift_gate"].squeeze(-1)
            predicted_scale = values["raw_predicted_scale"]
            predicted_shift = values["raw_predicted_shift"]
            raw_stage_forecasts = {
                "invariant": values["raw_invariant_forecast"],
                "shape": values["raw_after_shape_forecast"],
                "geometry": values["raw_after_geometry_forecast"],
                "final": values["raw_final_forecast"],
            }
            selected_probability = values["raw_selected_probability"]
            full_probability = values["raw_full_future_probability"]
            selected_entropy = -(
                selected_probability * selected_probability.clamp_min(1e-6).log()
            ).sum(-1)
            full_entropy = -(
                full_probability * full_probability.clamp_min(1e-6).log()
            ).sum(-1)
            logs.update({
                "raw_forecast/mapping_gate_mean": float(values["raw_mapping_gate"].mean().detach().cpu()),
                "raw_forecast/mapping_gate_std": float(values["raw_mapping_gate"].std(unbiased=False).detach().cpu()),
                "raw_forecast/delta_logits_magnitude": float(values["variant_future_logits"].norm(dim=-1).mean().detach().cpu()),
                "raw_forecast/full_probability_entropy": float(full_entropy.mean().detach().cpu()),
                "raw_forecast/selected_probability_entropy": float(selected_entropy.mean().detach().cpu()),
                "raw_forecast/effective_patterns": float(selected_entropy.exp().mean().detach().cpu()),
                "raw_forecast/top1_probability": float(full_probability.max(-1).values.mean().detach().cpu()),
                "raw_forecast/top3_cumulative_probability": float(
                    full_probability.topk(min(3, full_probability.shape[-1]), -1).values.sum(-1).mean().detach().cpu()
                ),
                "raw_forecast/top5_cumulative_probability": float(
                    full_probability.topk(min(5, full_probability.shape[-1]), -1).values.sum(-1).mean().detach().cpu()
                ),
                "raw_forecast/shape_gate_mean": float(shape_gate.mean().detach().cpu()),
                "raw_forecast/shape_gate_std": float(shape_gate.std(unbiased=False).detach().cpu()),
                "raw_forecast/shape_residual_norm": float(values["raw_shape_delta"].norm(dim=-1).mean().detach().cpu()),
                "raw_forecast/scale_gate_mean": float(scale_gate.mean().detach().cpu()),
                "raw_forecast/scale_gate_std": float(scale_gate.std(unbiased=False).detach().cpu()),
                "raw_forecast/predicted_scale_mean": float(predicted_scale.mean().detach().cpu()),
                "raw_forecast/predicted_scale_std": float(predicted_scale.std(unbiased=False).detach().cpu()),
                "raw_forecast/shift_gate_mean": float(shift_gate.mean().detach().cpu()),
                "raw_forecast/shift_gate_std": float(shift_gate.std(unbiased=False).detach().cpu()),
                "raw_forecast/predicted_shift_mean": float(predicted_shift.mean().detach().cpu()),
                "raw_forecast/predicted_shift_std": float(predicted_shift.std(unbiased=False).detach().cpu()),
                "raw_forecast/gate_zero_difference": float(
                    (values["raw_final_forecast"] - values["raw_invariant_forecast"]).abs().max().detach().cpu()
                ) if all(mode == "off" for mode in (
                    self.raw_shape_mode, self.raw_scale_mode, self.raw_shift_mode
                )) and self.raw_mapping_mode == "stable_only" else float("nan"),
            })
            for component, gate in (("shape", shape_gate), ("scale", scale_gate), ("shift", shift_gate)):
                for quantile, fraction in (("p10", .1), ("p50", .5), ("p90", .9)):
                    logs["raw_forecast/{}_gate_{}".format(component, quantile)] = float(_percentile(gate, fraction).cpu())
            for name, forecast in raw_stage_forecasts.items():
                logs["raw_forecast/{}_reconstruction_mse".format(name)] = float(
                    (forecast[:, -target.shape[1]:, -target.shape[2]:] - target).square().mean().detach().cpu()
                )
        return extra, logs
