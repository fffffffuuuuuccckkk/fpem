...
def _contingency(labels_left, labels_right):
    labels_left = np.asarray(labels_left, dtype=np.int64)
    labels_right = np.asarray(labels_right, dtype=np.int64)
    left_values, left_inverse = np.unique(labels_left, return_inverse=True)
    right_values, right_inverse = np.unique(labels_right, return_inverse=True)
    table = np.zeros((len(left_values), len(right_values)), dtype=np.int64)
    np.add.at(table, (left_inverse, right_inverse), 1)
    return table


def _adjusted_rand_index(labels_left, labels_right):
    table = _contingency(labels_left, labels_right).astype(np.float64)
    sample_count = table.sum()
    if sample_count < 2:
        return float("nan")
    choose2 = lambda value: value * (value - 1.0) / 2.0
    joint = choose2(table).sum()
    left = choose2(table.sum(1)).sum()
    right = choose2(table.sum(0)).sum()
    total = choose2(sample_count)
    expected = left * right / max(total, 1.0)
    maximum = 0.5 * (left + right)
    denominator = maximum - expected
    if abs(denominator) < 1e-12:
        return 1.0 if np.array_equal(labels_left, labels_right) else 0.0
    return float((joint - expected) / denominator)


def _normalized_mutual_info(labels_left, labels_right):
    table = _contingency(labels_left, labels_right).astype(np.float64)
    sample_count = table.sum()
    if sample_count <= 0:
        return float("nan")
    joint = table / sample_count
    left = joint.sum(1)
    right = joint.sum(0)
    expected = left[:, None] * right[None, :]
    mask = joint > 0
    mutual_information = np.sum(joint[mask] * np.log(joint[mask] / expected[mask]))
    left_entropy = -np.sum(left[left > 0] * np.log(left[left > 0]))
    right_entropy = -np.sum(right[right > 0] * np.log(right[right > 0]))
...
class PredictiveConflictEnvironment:
    def __init__(self, sample_count, env_num=6, steps=50, lr=.1, balance_weight=10.0,
                 ...
                 matching_gradient_weight=1.0, quality_diagnostics=False,
        ...
        self.quality_diagnostics = bool(quality_diagnostics)
        self.random_partition_repeats = int(random_partition_repeats)
        if self.random_partition_repeats <= 0:
            raise ValueError("random_partition_repeats must be positive")
        self.last_quality_payload = None
        self.update_index = 0
    ...
    def _random_partition_diagnostics(self, q, probe_per_sample, repeats):
        ...
        p_value = (1.0 + (values >= ours).sum()) / (float(repeats) + 1.0)
        return (
            float(ours),
            float(mean),
            float(std),
    ...
    def update(self, prediction, target, sample_id, source_split="train"):
        ...
        sample_mae = (pred - true).abs().flatten(1).mean(1)
        ...
        maes = torch.einsum("ne,n->e", self.q, sample_mae) / self.q.sum(0).clamp_min(1e-6)
        ...
        (
            ...
            random_p,
            random_conflicts,
        ) = self._random_partition_diagnostics(
            self.q,
            probe_per_sample,
            repeats=self.random_partition_repeats,
        )
        similarity_correlation = _corr(old_sim, new_sim)
        normalized_entropy = -(
            self.q * self.q.clamp_min(1e-12).log()
        ).sum(1).mean() / max(math.log(self.env_num), 1e-12)
        max_q = self.q.max(1).values
        previous_hard = previous.argmax(1).numpy()
        current_hard = self.q.argmax(1).numpy()
        if self.update_index == 0:
            stage_ari = float("nan")
            stage_nmi = float("nan")
        else:
            stage_ari = _adjusted_rand_index(previous_hard, current_hard)
            stage_nmi = _normalized_mutual_info(previous_hard, current_hard)
        separation_gain = (between - within) / max(within, 1e-8)
        record = {
            ...
            "random_partition_repeats": self.random_partition_repeats,
        ...
        if self.quality_diagnostics:
            record.update({
                "gradient_separation_gain": float(separation_gain),
                "random_partition_p_value": random_p,
                "normalized_assignment_entropy": float(normalized_entropy),
                "max_q_mean": float(max_q.mean()),
                "max_q_p10": float(torch.quantile(max_q, 0.1)),
                "max_q_p50": float(torch.quantile(max_q, 0.5)),
                "max_q_p90": float(torch.quantile(max_q, 0.9)),
                "stage_ARI_to_previous": stage_ari,
                "stage_NMI_to_previous": stage_nmi,
                "per_env_MSE": risks.tolist(),
                "per_env_MAE": maes.tolist(),
                "per_env_mean_sample_loss": risks.tolist(),
                "env_gradient_scale": gradients[:, 0].tolist(),
                "env_gradient_bias": gradients[:, 1].tolist(),
            })
            self.last_quality_payload = {
                "sample_id": sample_id.numpy(),
                "g_scale": probe_per_sample[:, 0].numpy(),
                "g_bias": probe_per_sample[:, 1].numpy(),
                "q": self.q.numpy(),
                "hard_env": current_hard,
                "sample_mse": sample_loss.numpy(),
                "sample_mae": sample_mae.numpy(),
                "environment_centroids": gradients.numpy(),
                "random_partition_conflicts": random_conflicts,
                "conflict_ours": np.asarray(conflict),
            }
