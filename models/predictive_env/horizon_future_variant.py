...
class HorizonFutureVariant(nn.Module):
    ...
    def __init__(
        ...
        self.environment_correction_heads = None
        if self.environment_count > 1:
            self.environment_correction_heads = nn.ModuleList(
                [
                    nn.Sequential(
                        nn.LayerNorm(self.d_model),
                        nn.Linear(self.d_model, int(bottleneck)),
                        nn.GELU(),
                        nn.Linear(int(bottleneck), self.future_patch_len),
                    )
        ...
        if self.environment_correction_heads is not None:
            for head in self.environment_correction_heads:
    ...
    def forward(self, invariant_prediction, z_inv, z_var):
        ...
        environment_corrections = None
        environment_disagreement = None
        if self.environment_correction_heads is not None:
            # These estimates are an isolated reliability feature: their input
            # is detached so their supervised loss cannot reshape Future-Zvar.
            estimates = torch.stack(
                [head(future.detach()) for head in self.environment_correction_heads],
                dim=1,
            )
            environment_corrections = estimates
            environment_disagreement = estimates.var(
        ...
        if environment_disagreement is not None:
            reliability_parts.append(
                torch.log1p(environment_disagreement).unsqueeze(-1)
            )
        reliability_input = torch.cat(reliability_parts, dim=-1).detach()
        ...
        for patch_index in range(self.future_patch_count):
            ...
            patch_variation = (
                raw_variation
                if self.training or not self.use_reliability_gate
                else gated_variation
            )
        ...
        return {
            ...
            "environment_corrections": environment_corrections,
            "environment_disagreement": environment_disagreement,
        }
